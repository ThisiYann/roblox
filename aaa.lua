...
-- Anti-Idle System
for i, connection in ipairs(getconnections(LocalPlayer.Idled)) do
    connection:Disable()
end
...
local function checkStatus()
    ...
    if response == "UPDATE" then
        if not CheckData.kicked then
            ...
            LocalPlayer:Kick("ArtHub Update Available!.")
        end
    elseif response == "LATEST" then
        if not CheckData.notified then
            CheckData.notified = true
            warn("[ArtHub] Status: Latest version")
        end
    else
        warn("[ArtHub] Status unknown:", response)
    end
end
...
local Shop = AllMenu:Tab({
    Title = "Shop",
    Icon = "shopping-bag"
})

local Tele = AllMenu:Tab({
    Title = "Teleport",
    Icon = "map-pin"
})
...
Tele:Button({ Title = "Refresh Players", Icon = "refresh-cw", Callback = refreshInventory })
